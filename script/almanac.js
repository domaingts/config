export default async function (ctx) {
    ...
    const Lunar = {
        ...
        parse(y, m, d) {
            ...
            const start = m * 2 - (d < [20, 19, 21, 21, 21, 22, 23, 23, 23, 23, 22, 22][m - 1] ? 2 : 0);
            const astro = "摩羯水瓶双鱼白羊金牛双子巨蟹狮子处女天秤天蝎射手摩羯".slice(start, start + 2) + "座";
    ...
    for (let i = 0; i < allTerms.length; i++) {
        ...
        if (diff >= 0) {
            currentTerm = diff === 0 ? allTerms[i].name : (i > 0 ? allTerms[i - 1].name : Lunar.termNames[23]);
    ...
    const splitText = (str, maxW = FIRST_LINE_MAX_CHARS) => {
        ...
        for (const token of (str.match(/[\d\/a-zA-Z.\-]+|./gu) || [])) {
            const tw = [...token].reduce((s, c) => s + (/[\u2E80-\u9FFF\uF900-\uFAFF]/.test(c) ? 2 : 1.1), 0);
            if (w + tw > maxW) {
                lines.push(line);
                line = token;
                w = tw;
    ...
    const buildRow = (icon, color, label, content, isFirst = true, contentColor = C.sub) => ({
        ...
        children: [
            ...(isFirst ? [
                {
                    type: 'stack', direction: 'row', alignItems: 'center', gap: 2, width: LABEL_COL_WIDTH, children: [
                        mkIcon(icon, color),
                        mkText(label, 12, "heavy", color)
                    ]
                }
            ] : [
                { type: 'stack', width: LABEL_COL_WIDTH, children: [] }
            ]),
    ...
    const buildYiJiRows = (raw, icon, color, label) => {
        if (!raw)
            return [];
        const [l0, ...rest] = splitText(raw, FIRST_LINE_MAX_CHARS);
        const rows = [];
        if (l0)
            rows.push(buildRow(icon, color, label, l0, true));
        if (rest.length)
            rows.push(buildRow(icon, color, label, rest.join(" "), false));
        return rows;
    };
    const topIcon = SHOW_MODE === 'week' ? 'list.number' : 'sparkles';
    const topText = SHOW_MODE === 'week' ? getWeekInfo(now) : obj.astro;
    const nextMidnight = new Date(Y, M - 1, D + 1);
    if (ctx.widgetFamily === 'accessoryRectangular') {
        return {
            type: 'widget', padding: 4,
            children: [
                mkText(`${M}月${D}日 星期${WEEK}`, 'headline', 'bold', '#FFFFFF'),
                mkText(`${obj.cn} ${obj.gz}年`, 'caption1', 'medium', '#FFFFFFCC')
            ]
        };
    }
    else if (ctx.widgetFamily === 'accessoryCircular') {
        return {
            type: 'widget', padding: 2,
            children: [
                mkText(`${D}`, 'title1', 'bold', '#FFFFFF'),
                mkText(`${obj.cn}`, 'caption2', 'medium', '#FFFFFFCC')
            ]
        };
    }
    else if (ctx.widgetFamily === 'accessoryInline') {
        return {
            type: 'widget',
            children: [
                mkText(`${M}/${D} ${obj.cn} ${shichenStr}`, 'body', 'medium', '#FFFFFF')
            ]
        };
    }
    if (ctx.widgetFamily === 'systemSmall') {
        return {
            ...
            children: [
                mkText(`${M}月${D}日 周${WEEK}`, 15, "heavy", C.main),
                { type: 'spacer', length: 4 },
                mkText(`${obj.gz}(${obj.ani})年`, 13, "bold", C.gold),
                mkText(`${obj.cn} ${shichenStr}`, 13, "bold", C.gold),
                { type: 'spacer', length: 4 },
                ...(rawYi ? [mkRow([mkIcon('checkmark.circle.fill', C.yi, 11), mkText(rawYi.split(/\s+/).slice(0, 3).join(" "), 11, "medium", C.yi)], 3)] : []),
                ...(rawJi ? [mkRow([mkIcon('xmark.circle.fill', C.ji, 11), mkText(rawJi.split(/\s+/).slice(0, 3).join(" "), 11, "medium", C.ji)], 3)] : []),
                { type: 'spacer' }
            ]
        };
    }
        ...
        children: [
            ...
            { type: 'spacer', length: 6 },
            {
                type: 'stack', direction: 'column', alignItems: 'start', gap: 6, children: [
                    mkText(`${obj.gz}(${obj.ani})年 ${obj.cn} ${shichenStr}${obj.term ? ` · 今日${obj.term}` : ` · 当前${currentTerm}`}`, 13, "bold", C.gold),
                    ...buildYiJiRows(rawYi, 'checkmark.circle.fill', C.yi, '宜'),
                    ...buildYiJiRows(rawJi, 'xmark.circle.fill', C.ji, '忌'),
                    ...(fetchError ? [
                        mkRow([mkIcon('exclamationmark.triangle', C.muted, 11), mkText('数据加载失败，请检查网络', 11, 'medium', C.muted)], 3)
                    ] : []),
